	...
	public static partial class wld
	{
						public class CStateWorldMove : CBaseState
						{
							...
							public void sendMenu(CBaseSystem _sys)
							{
								...
								// ... or a mod asked for one of its own screens (Game.Menus.Open): the menu opens and shifts to it.
								if (!OpenFF.Client.GameProfile.IsFf4 && ((ds.g_Pad.edge() & 0x400) != 0 || (ds.g_Pad.edge() & num) != 0 || _sys.World2DMng().MenuStartButton().isTouch() || OpenFF.Client.ModMenus.MenuRequested()))
								{
									_sys.World2DMng().refMapNameWindow().close();
