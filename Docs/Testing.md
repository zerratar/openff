...
| C-12 | Quicksave | Menu ▸ Quicksave ▸ Yes | The first tap on Yes moves the hand to it, the second confirms: the screen fades and comes back, and `save.bin` under `%LocalAppData%\IsolatedStorage` has a new modified time. One tap does nothing but move the hand - that is the phone's two-step, not a bug |
