...
| C-15 | Mods by game and target | Give one mod a `mod.json` with `"games": ["ff4"]`, another `"target": "steam"` | Under FF3 neither applies and the log says `for ff4` and `targets steam`; `--game=ff4` applies the first. A mod folder without any `mod.json` still loads, named after its folder |
