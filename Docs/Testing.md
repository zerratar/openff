...
| D-1 | FF4 boots into Baron | `FF3.exe --content="C:\Program Files (x86)\Steam\steamapps\common\Final Fantasy IV" --size=1600x960` | Within ~10 s: Baron castle's grounds (stone platform, moat, bridge) with Cecil in dark-knight armour and his shadow, at the world-map arrival; the castle theme plays; the log says `content: loose files + mass files: 11361 files ...; then loose files (2 files)` and `script: FF4 table built - 200 of 500 commands` |
