...
| `GameCast` | Cast; Treasure, Item, Gil, Flag; Recolour | the object *is* that cast of the map's script: talking to it runs `cast<N>_main` and the script's commands on cast N land on it (`Npc.RunCast`); a chest's treasure the game's way (`Npc.SetTreasure`); a texture variant (`Npc.Recolour`). What Crystal's exact conversion puts on a stand-in - the same code on the same kind of character, 1:1 |
