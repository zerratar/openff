...
| | fields | does |
| --- | --- | --- |
| `Chest` | Item (picked from the game's list), Count, Gil; Once, Message, EmptyMessage, Flag, ChestLook | gives the contents and says so when the player talks to it (a model: face it and press A, the game's own talk), or, without one, presses A standing within Radius - approach, then interact; OnWalkIn makes it act the moment the hero walks in instead. Once remembers it across saves; Flag is the game's own chest flag; ChestLook plays the lid, the sound and the sparkle as the game does |
...
| `GameCast` | Cast, OnBoot; Treasure, Item, Gil, Flag; Recolour | the object *is* that cast of the map's script: talking to it runs `cast<N>_main` and the script's commands on cast N land on it (`Npc.RunCast`); a chest's treasure the game's way (`Npc.SetTreasure`); a texture variant (`Npc.Recolour`). *OnBoot*: a scene's actor - not spawned with the map but when the script boots the cast (`Events.CastBooted`), where it boots it, and driven by the scene from there. What Crystal's exact conversion puts on a stand-in - the same code on the same kind of character, 1:1 |
