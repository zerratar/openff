...
internal static partial class GlobalScope
{
	...
	{
		public class BattleActionKnockOverMoveFront : BattleActionBase
		{
			public override void initialize(BattlePlayer player)
			{
				...
				player.speed_set(PLAYER_MOVE_DISTANCE / ((num2 - num3 + 1 == 0) ? 1 : (num2 - num3 + 1)));
				rotateFront(player);
			}
			...
			public override bool execute(BattlePlayer player)
			{
				...
				fnd_reuse_pos.x = vecFx.x - -FX_SinIdx(player.moveYaw()) * PLAYER_MOVE_DISTANCE * 2 / 4096;
				fnd_reuse_pos.y = 0;
				fnd_reuse_pos.z = vecFx.z - FX_CosIdx(player.moveYaw()) * PLAYER_MOVE_DISTANCE * 2 / 4096;
