...
internal static partial class GlobalScope
{
	...
	{
		public class BattleActionItem : BattleActionBase
		{
			public override void initialize(BattlePlayer player)
			{
				if (!player.condition().isFrog())
				{
					characterMng.startMotion(player.characterMngId(), 701, fLoop: false, 0u);
				}
			}
			...
			public override bool execute(BattlePlayer player)
			{
				...
				if (motionIndex == 701 && characterMng.isEndOfMotion(player.characterMngId()))
				{
					return true;
				}
				return false;
